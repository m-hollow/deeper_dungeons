...
def battle_main(player, monster, run_failed):
	...
	while active:
		...
		if active:
			...
			# reset the fight mods to None so each round of battle uses blanked mods.
...
def player_attack(player, monster, fight_mods, round_num, crits):
	...
	# designed so that mods for attack area choice don't crew with class objects; 
	# if they did, you'd have to reset them, which is annoying and probably inefficient.
