	...
	while active:
		...
		if command not in possible_choices:
			...
			if command.lower().startswith('f') or active:
				...
				active = False
				battle_shortloop(player, monster, run_failed)
...
def battle_shortloop(player, monster, run_failed):

	# this version among other things attempts to get rid of monster.advantage
	# is also makes sure there is essentially only ONE call to press_enter on an entire loop pass.
	# technically there are two in the code: 1 at end of loop if loop continues, 1 at end if loop ends.

	round_num = 1
	fight_mods = {'enemy_armor': 0, 'player_damage': 0}
	crits = {'crit': False}
	player_turn = True
	...
	while active:
		...
		battle_header(player, monster, round_num)	# this should be the ONLY call to this function!
		...
		if player_turn:
			...
			if player_attack(player, monster, fight_mods, round_num, crits):
				if not crits['crit']:
					...
					press_enter()	# is this neccessary if you have a press_enter call at end of loop?
		...
		if active:
			...
			round_num += 1		# oh shit this is updating after every -individual- round now...fix this.
...
def player_attack(player, monster, fight_mods, round_num, crits):
	...
	# so, we use this while: it loops until a *non-looping* command is given, such as an attack.
	while active:
