...
class GameGrid():
	...
	def find_current_room(self, all_rooms, room_to_find):
		...
		# by matching coordinates with the first grid, which is exactly what this loop does.

		# QUESTION, but if you have the coords of the player on the surface grid, and the room grid is the 
		# same size, why can't you just literally use the coords to access the room type? do you really need
		# to loop through everything? isn't there just a way to say "here's the coords, give me the room type
		# for the dict that contains those coords"


		# it's already determined what every room is when the grid is generated, and every room has coords, and
		# those coords are the same as the  'surface' board, so why not just grab the room type directly based
		# on the coords? well, what does that mena, 'grab directly'? how would you access the type? it's a value
		# in a dictionary...maybe the issue is that 'Coords' shouldn't be a Key: Value pair at all, it's redundant
		# to do that (and thus John's question about why it's there) -- coords are just a thing you access directly
		# in the code to grab values at those coords. you're abstracting them into a dictionary Key: Value pair, but
		# for no good reason. this is why yo then have to loop through the dictionary to find the match between
		# the literal coords from the surface grid and the Key: Value coords in the dictionary in the second (room) grid. 
		# you're getting at something here in this "grab it directly" idea; you're saying, just use the coords literally
		# to grab an item stored at the coordinate location, not some indirect matching of a key value that is the coords.
		# what is stored at [3,3] in the surface grid? a ' X '. What is stored at [3,3] in the room grid? a literal
		# string with the name of the room type. or, a list with the room type and other info. or even a dictionary
		# with room type and visited boolean; but all that info just exists at those coordinates [3,3], you don't need
		# it inside the dictionary as a key: value pair, doing so is actually a redundancy that is making the program
		# more complicated. you over-thought it !!

		# see what I mean? you created a second grid. it has coordinate locations, just like the initial '*' grid does.
		# why store those coordinates at each location? why not just store the info you need at each coordinate, and 
		# grab it as necessary based on the same coord value from the first grid?
