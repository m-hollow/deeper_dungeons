...
class GameGrid():
	...
	def create_exit(self): # is there a simpler / more efficient way to do this? 
		...
		while active:
			...
			if coords != self.player.player_location:	# works because this function is only called on object creation, when player loc = start
...
grid_one = GameGrid(settings, player) # see new location in main_moves_two.py
game_log = GameLog(grid_one)		  # see new location in main_moves_two.py
